        ...
        <el-main>
            <div class="post-list-view">
                <user-post v-for="post in postList" :key="post.post_id" :post="post" @userCommentUpdate="updatePost"></user-post>
                 <div v-if="postList.length==0" class="no-data">暂无评论记录</div>
            </div>
        </el-main>
        <el-aside>
             <div class="user-card">
...
export default {
  ...
  created(){
    ...
    methods:{
      ...
      axios.post('http://localhost:3004/community/getUserPostComment',{
            userId
      })
      .then((response)=>{
